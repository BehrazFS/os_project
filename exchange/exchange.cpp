...
void request_handler() {
    while (true) {
        ...
        if (data == "GET_PRICE_CLIENT") {
            ...
            string message = "GET_CRYPTO_PRICE " + cyripto_name +" : " + to_string(safe.cryptocurrencies[cyripto_name].price);
            ...
            if (!safe.cryptocurrencies.contains(cyripto_name)) {
                ...
                // cout<<si.count<<endl;
                if (((si.creation_time + chrono::seconds(30) <= chrono::system_clock::now()) && si.state == "preorder") || (si.state == "preorder" && si.price >= 900)) {
                    si.state = "released";
                    //...
                    ...
                    if (amount > si.count && si.state == "released") {
                        ...
                        bank_server_addr.sin_port = htons(BANK_PORT);
                        inet_pton(AF_INET, server_ip.c_str(), &bank_server_addr.sin_addr);
                        string message2 = "BUY_CRYPTO_CLIENT_CHECK " + cyripto_name +" " + to_string(amount) + " " + to_string(si.price) + " " + to_string(client_port) + " " +to_string(assigned_port);
                        message2 = simpleHash(message2) + " " + message2;
                        sendto(bank_sock_fd, message2.c_str(), message2.size(), 0, (const struct sockaddr *)&bank_server_addr, sizeof(bank_server_addr));
                        close(bank_sock_fd);
                        cout << "exchange sent to bank for check"<<endl ;
                    }
                }
            }
            if (send) {
...
void new_crypto_handler() {
    while (true) {
        ...
        message = simpleHash(message) + " " + message;
        sendto(bank_sock_fd, message.c_str(), message.size(), 0, (const struct sockaddr *)&bank_server_addr, sizeof(bank_server_addr));
        close(bank_sock_fd);
        this_thread::sleep_for(chrono::seconds(2));
    }
}
